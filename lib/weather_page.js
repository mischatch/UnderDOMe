...
const cityForcast = forc => {
  ...
  let forecast = {
    ...
    weather: `http://openweathermap.org/img/w/${forecastObj.weather[0].icon}.png`
  };
  return (
    `<li>
...
const forecastWidget = (data) => {
  return (
    `<div class="weather-el">
      ...
      <ul class="invis">
        <li>Temp.<br> ${data.temp}<span class="degree-symbol">°</span><span>C</span></li>
        <li>Pres.<br> ${data.pressure}</li>
        <li>Hum.<br> ${data.humidity}%</li>
        <li>Weather<br> <img src="${data.weather}" /></li>
      </ul>
